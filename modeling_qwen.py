...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.use_flash_attn = False # disable flash_attn
    ...
    def _attn(self, query, key, value, registered_causal_mask, attention_mask=None, head_mask=None, next_mask=None):
        device = query.device
        ...
        if next_mask is not None:
            bs, n_head, q_len, k_len = attn_weights.shape
            attn_weights = attn_weights.view(bs * n_head, k_len)
            for i in range(bs * n_head):
                attn_weights[i, next_mask[i]] = torch.finfo(attn_weights.dtype).min
            attn_weights = attn_weights.view(bs, n_head, q_len, k_len)
    ...
    def forward(
        ...
        if (
            ...
            if not self.use_cache_quantization and False:
                causal_mask = registered_causal_mask[
                ...
                attn_output, attn_weight = self._attn(
                    query, key, value, registered_causal_mask, attention_mask, head_mask, self.next_mask
                )
        ...
        if perform_kv_evict:
            ...
            _, mask_topk = current_scores_sum.topk(k = (attn_weight.shape[-1] - heavy_budget), dim = -1, largest = False, sorted = True)

            self.next_mask = mask_topk
            ...
            # present = (torch.cat([key[:,:300,:,:],key[:,-300:,:,:]],dim=1), torch.cat([value[:,:300,:,:],value[:,-300:,:,:]],dim=1))
...
class QWenBlock(nn.Module):
    ...
    def forward(
        ...
        if perform_tome:
            ...
            end_time = time.time()
            # print(f"perform token merge on layer {self.layer_id}")
            print(f"original audio seq len: {orig_seq_len}, new audio seq len: {new_audio_seq_len}, actual merge ratio: {1 - new_audio_seq_len/orig_seq_len}, perform merge time: {end_time - start_time}")
